...
export class PostDetailsComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  private currentPostSubscribtion: Subscription;
  private relatedPostsSubscribtion: Subscription;
  ...
  ngOnInit() {
    ...
    this.relatedPostsSubscribtion = this.postService.getRelatedPostsUpdateListener()
      .subscribe((data) => {
  ...
  onRelatedPostSelected(post) {
    ...
    this.postService.getPost(post._id, this.paginator.itemsPerPage, this.paginator.currentPage);
  ...
  ngOnDestroy() {
    ...
    this.currentPostSubscribtion.unsubscribe();
    this.relatedPostsSubscribtion.unsubscribe();
  }
}
